...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        // 🔥🔥 全系 4:3 采集（1440x1080），输出分辨率保持不变
        // scale=1:       1440x1080 (最高档)
        // scale=1.125:   1280x960  (超清档)
        // scale=1.5:     960x720   (高清档)
        // scale=2:       720x540   (标清档)

        if device.position == .back {
            // 🎥 后置摄像头：4档固定（采集 1440x1080 @60fps）
            currentLadder = [
                .p4k:      LadderPreset(width: 1440, height: 1080, fps: 60, maxKbps: 4500, maxPushFps: 60),   // scale=1
                .ultra:    LadderPreset(width: 1280, height: 960,  fps: 60, maxKbps: 3000, maxPushFps: 60),   // scale=1.125
                .high:     LadderPreset(width: 960,  height: 720,  fps: 60, maxKbps: 2000, maxPushFps: 60),   // scale=1.5
                .standard: LadderPreset(width: 720,  height: 540,  fps: 60, maxKbps: 1500, maxPushFps: 60)    // scale=2
            ]
            print("📐 后置摄像头 - 全系4:3配置（1440x1080采集）：")
            print("   最高  = 1440x1080 (scale=1)     → 4500kbps")
            print("   超清  = 1280x960  (scale=1.125) → 3000kbps")
            print("   高清  = 960x720   (scale=1.5)   → 2000kbps")
            print("   标清  = 720x540   (scale=2)     → 1500kbps")
        } else {
            // 📱 前置摄像头：4档固定（采集 1440x1080 @60fps）
            currentLadder = [
                .p4k:      LadderPreset(width: 1440, height: 1080, fps: 60, maxKbps: 4500, maxPushFps: 60),   // scale=1
                .ultra:    LadderPreset(width: 1280, height: 960,  fps: 60, maxKbps: 3000, maxPushFps: 60),   // scale=1.125
                .high:     LadderPreset(width: 960,  height: 720,  fps: 60, maxKbps: 1500, maxPushFps: 60),   // scale=1.5
                .standard: LadderPreset(width: 720,  height: 540,  fps: 60, maxKbps: 1200, maxPushFps: 60)    // scale=2
            ]
            print("📐 前置摄像头 - 全系4:3配置（1440x1080采集）：")
            print("   最高  = 1440x1080 (scale=1)     → 4500kbps")
            print("   超清  = 1280x960  (scale=1.125) → 3000kbps")
            print("   高清  = 960x720   (scale=1.5)   → 1500kbps")
            print("   标清  = 720x540   (scale=2)     → 1200kbps")
        }
    }
    ...
    private func applyShutterSpeedChange() {
        ...
        // 🔥 快门范围 120~240（cjfps 0%=1/120s, 100%=1/240s）
        let targetShutterSpeed = 120 + Int(Double(cjfpsPercentage) * 1.2)  // 120~240
        ...
        do {
            ...
            let fastestShutter = Int(round(1.0 / CMTimeGetSeconds(minDur)))
            let slowestShutter = Int(round(1.0 / CMTimeGetSeconds(maxDur)))
            print("📷 设备快门范围: 1/\(fastestShutter)s ~ 1/\(slowestShutter)s")
            ...
            if device.isExposureModeSupported(.custom) {
                ...
                // 🔥 iOS 18+ 安全处理：确保ISO在有效范围内
                var safeISO = device.iso
                let minISO = device.activeFormat.minISO
                let maxISO = device.activeFormat.maxISO
                if safeISO < minISO || safeISO > maxISO || safeISO.isNaN || safeISO.isInfinite {
                    // ISO无效，使用中间值
                    safeISO = (minISO + maxISO) / 2
                    print("⚠️ ISO无效，使用安全值: \(safeISO)")
                }
    ...
    func startPreviewIfNeeded(initialProfile: LadderProfile? = nil) {
        ...
        if let cfg = ConfigManager.shared.getCurrentConfig() {
            ...
            if let serverFps = cfg.fps {
                // 🔥 后端下发的是采集fps，推送fps = 采集fps / 2
                let pushFps = serverFps / 2
                targetOutputFPS = pushFps
                print("🎯 [初始化-预览] 从缓存读取FPS: 后端=\(serverFps)fps / 2 = 推送目标=\(pushFps)fps")
            } else {
                print("⚠️ [初始化-预览] 缓存无FPS，使用默认值: \(targetOutputFPS)fps")
            }
        } else {
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 固定采集分辨率（4:3 比例）
    // 全系使用 1280x960 (4:3) 采集，更低分辨率支持更快快门
    private let fixedCaptureWidth: Int = 1440
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let serverCfg = ConfigManager.shared.getCurrentConfig() {
            ...
            if let serverFps = serverCfg.fps {
                ...
                let pushFps = serverFps / 2
                targetOutputFPS = pushFps
                print("🎯 [初始化-推流] 从缓存读取FPS: 后端=\(serverFps)fps / 2 = 推送目标=\(pushFps)fps")
            } else {
    ...
    private func configureCameraAutoModes(_ device: AVCaptureDevice) {
        do {
            ...
            // 🔥🔥 快门速度由 cjfps 控制（0-100 → 90-900）
            // cjfps=0 → 1/90s, cjfps=100 → 1/900s
            if device.isExposureModeSupported(.custom) {
                // 打印设备支持的快门范围
                let minDur = device.activeFormat.minExposureDuration
                let maxDur = device.activeFormat.maxExposureDuration
    ...
    func getResolutionScaleForProfile(_ profile: LadderProfile) -> Double {
        ...
        // 🔥🔥 全系 4:3 采集（1280x960），缩放到目标分辨率
        // 4K/超清: scale=1 (1280x960)
        // 高清: scale=1.33 (960x720)
        // 标清: scale=2 (640x480)
